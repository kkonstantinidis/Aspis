...
# scp -i ${KEY_PEM_DIR} -r "/home/kostas/Dropbox/Python Workspace/SGD/ASPIS" ubuntu@${PUB_IP_ADDR}:~/
...
rsync -avzh -e "ssh -i ${KEY_PEM_DIR}" ~/Dropbox/Python\ Workspace/SGD/ASPIS/ ubuntu@${PUB_IP_ADDR}:~/ASPIS${PROJECT_INDEX} --exclude '*data' --exclude '__pycache__'
# ssh -i ${KEY_PEM_DIR} ubuntu@${PUB_IP_ADDR} 'sudo apt-get update; cp ASPIS/tools/remote_script.sh ~/'
