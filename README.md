...
| `adversarial-detection` | Method used to detect and potentially exclude adversaries: `clique` (if a unique maximum clique of size K-q exists all other workers will be ignored). The surviving gradients will be aggregates with a `coord-median`-type aggregator. If set to anything else, no detection will take place.  |
