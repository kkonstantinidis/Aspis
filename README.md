...
| `err-choice` | How adversaries choose which files to distort: `fixed_disagreement` (all q adversaries disagree with a fixed set of honest workers), `all` to distort all files. For now, it works only if ALIE is disabled (TBD). |
